...
class PublicChannelAutoJoinerTaskTest {
	...
	@Test
	void testTask() throws InterruptedException, RestClientException, ReplyErrorException, IOException {
		// given
		...
		PublicChannelAutoJoinerTask task = new PublicChannelAutoJoinerTask(restClient, rtClient, bot, authInfoHolder,
				10L);
		...
		verify(rtClient, atLeast(4)).sendMessageAndWait(captor.capture(), eq(ReceiveJoinRoomReply.class));
